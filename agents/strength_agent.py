...
class StrengthAgent():
    ...
    def __init__(self, mu: float, parents=None):
        ...
        self.dna = np.random.choice(StrengthAgent.nucleotides, 135) if parents is None else self.inherit(parents, mu)
        self.malintent, self.communication, self.strength = self.features_from_dna(self.dna)
    ...
    def inherit(self, parents: tuple, mu: float):
        ...
        for i in range(len(child_dna)):
            if st.bernoulli(mu).rvs(1):
                child_dna[i] = random.choice(StrengthAgent.nucleotides)
        return child_dna

    # extract features from DNA
    def features_from_dna(self, dna: list):
        communication, malintent, strength = 0, 0, 0
        for i in range(0, len(dna), 3):
            if i < (len(dna)/3):
                ...
                if "".join(dna[i:i+3])=="cat":
                    strength += 1
        return (malintent, communication, strength)
            ...
            if st.bernoulli(other.prior_communication).rvs(1):
                ...
                if st.bernoulli(self.prior_malintent).rvs(1):
                    ...
                    if not st.bernoulli(other.prior_malintent).rvs(1):
                        self.food_counter += StrengthAgent.food_stealing
                    # other also wants to steal the food:
                    else:
                        if st.bernoulli(self.strength/(self.strength/other.strength)).rvs(1):
                            self.food_counter += StrengthAgent.food_stealing
                        else:
                            other.food_counter += StrengthAgent.food_stealing
                else:
                    if st.bernoulli(other.prior_malintent).rvs(1):
                        other.food_counter += StrengthAgent.food_stealing
                    else:
                        # share food:
                        self.food_counter += StrengthAgent.food_sharing
                        other.food_counter += StrengthAgent.food_sharing
                        # increase reputation:
                        self.reputation += 1
                        other.reputation += 1
            else:
                self.food_counter += StrengthAgent.food_individual_consumption
        else:
            self.food_counter += StrengthAgent.food_individual_consumption
    ...
    def apply_food_decay(self):
        self.food_counter -= StrengthAgent.food_decay

    def __str__(self):
        return(
            f"StrengthAgent (Parents: {self.parents})\n"
            "##### Features #####\n"
